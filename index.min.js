...
var siteUrl = "https://zeno.fm"
  , stationPrettyUrl = window.location.pathname.split("/")[2];
...
$(document).ready(function() {
    "radiohosannaonline" === stationPrettyUrl && (document.getElementById("playstore").remove(),
    document.getElementById("appstore").remove()),
    $(".btn-play").click(function() {
        let t = document.getElementById("audioPlayer");
        if ($(this).hasClass("paused"))
            t.pause();
        else {
            let e = "";
            void 0 !== window.adtonosListenerId && window.adtonosListenerId.length && (e = "?adtonosListenerId=" + window.adtonosListenerId);
            try {
                let a = com_adswizz_synchro_getListenerId();
                a && a.length && (e += (e.length ? "&" : "?") + "aw_0_req_lsid=" + a)
            } catch (n) {}
            getPartnerIds().then(t=>{
                let a = new URLSearchParams(t).toString();
                e += (e.length ? "&" : "?") + a
            }
            ).catch(t=>{}
            ).finally(()=>{
    ...
    fetchStation(stationPrettyUrl).done(function(t) {
        ...
        document.getElementById("audioPlayer").src = e.streamUrl;
        let a = new EventSource("https://api.zeno.fm/mounts/metadata/subscribe/" + e.streamName);
        a.onmessage = function(t) {
            let e = JSON.parse(t.data);
            e.streamTitle && $(".dashboard-box__playing-song").text(e.streamTitle)
        }
    })
});
